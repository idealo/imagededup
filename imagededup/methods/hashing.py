...
from imagededup.utils.image_utils import load_image, preprocess_image # check_directory_files
...
class Hashing:
    ...
    def hash_func(self, image_array):
    ...
    def _save_results(self, filename) -> None:
        ...
        <distance>,..}}
        """
        self.logger.info('Start: Saving duplicates as json!')
        with open(filename, 'w') as f:
            json.dump(self.results, f, indent=2, sort_keys=True)
        self.logger.info('End: Saving duplicates as json!')
    ...
    def _check_hamming_distance_bounds(thresh: int) -> None:
        ...
        if not isinstance(thresh, int) or ():
...
class PHash(Hashing):
    ...
    def hash_algo(self, image_array):
        ...
        dct_reduced_coef = dct_coef[:self.coefficent_extract[0], :self.coefficent_extract[1]]
        ...
        return dct_reduced_coef >= mean_coef_val
